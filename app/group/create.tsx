// Tạo nhóm — port từ web (views/groups/GroupsCreate.tsx):
// chọn ảnh đại diện / ảnh bìa trước, tạo nhóm rồi upload ảnh cho nhóm vừa tạo.
...
export default function CreateGroupScreen() {
  const colors = useThemeColors();
    ...
    const handleCreate = async () => {
        ...
        if (newGroup) {
            // Upload ảnh sau khi có groupId (như web). Lỗi upload không chặn việc vào nhóm.
    ...
    return (
        <SafeAreaView className="flex-1 bg-background" edges={['top']}>
            ...
            <ScrollView className="flex-1" contentContainerClassName="p-4 gap-6" keyboardShouldPersistTaps="handled">
                {/* Ảnh bìa + ảnh đại diện — chọn trước, upload sau khi tạo nhóm (như web) */}
                <View className="gap-2">
                    <Text variant="large" className="font-semibold">Ảnh nhóm</Text>
                    <Pressable
                        ...
                        {cover ? (
                            <Image source={{ uri: cover.uri }} style={{ width: '100%', height: 140 }} contentFit="cover" />
                        ) : (
                            <View className="h-[140px] items-center justify-center gap-1">
                                <Ionicons name="image-outline" size={26} color={colors.mutedForeground} />
                                <Text variant="small" className="text-muted-foreground">Thêm ảnh bìa</Text>
                            </View>
                        )}
                    </Pressable>
                    ...
                    <View className="-mt-8 flex-row items-end gap-3 px-3">
                        <Pressable
                            ...
                            <View className="size-16 overflow-hidden rounded-full border-2 border-card bg-muted">
                                {avatar ? (
                                    <Image source={{ uri: avatar.uri }} style={{ width: '100%', height: '100%' }} contentFit="cover" />
                                ) : (
