// Entry point lớp storage — port từ web (src/plugins/storage/index.ts).
// Chọn provider theo cấu hình, có fallback nếu provider ưu tiên chưa cấu hình đủ.
...
// Mặc định "r2" — khớp với web (VITE_STORAGE_PROVIDER || "r2").
...
// Sinh tên object duy nhất, giữ đuôi tệp gốc (giống buildObjectPath của web).
...
// Upload một tệp lên provider đang hoạt động.
